...
- **2026-05-30**: Resolved Batch 2 Issues (Drift Joining, Participant State & Details). Fixed UI gaps/spaces in `DriftDetailScreen` by removing redundant bottom spacer and hero top padding. Fixed the empty participant list sheet (ISSUE-045) by fetching live message threads and mapping profiles, falling back to mock details. Made participant row card fully clickable post-join (ISSUE-046). Displayed unlocked precise coordinates inside `WhoIsComingSheet` (ISSUE-047). Corrected CTA button so the last user to join a full Drift sees "Open Chat" instead of "Drift Locked" (ISSUE-048). Verification: not run here (per repo rules); validate in Xcode.
