...
- Refactored Coffee Header System with Generic Swift-Native Design:
  ...
  - Overhauled `CoffeeHeaderButton` to be fully auto-sizing and fluid. Replaced fixed frame bounds with dynamic padding, `minWidth`/`minHeight` (defaulting to `44` pt for comfortable tap targets), and `maxWidth` limits, and wrapped them in `Capsule` shapes so the buttons scale smoothly and automatically depending on their contents.
