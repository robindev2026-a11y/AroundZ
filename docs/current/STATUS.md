...
- Refactored Coffee Header System with Generic Swift-Native Design:
  ...
  - Overhauled `CoffeeHeaderButton` to be fully auto-sizing and fluid. Replaced fixed frame bounds with dynamic padding, `minWidth`/`minHeight` (using user's custom `minSize = 20` pt), and `maxWidth` limits, and wrapped them in `Circle` shapes (maintaining user's customized design language).
