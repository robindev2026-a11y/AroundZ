...
public class JoinRequestDebugTracer {
    public static func trace(_ message: String, driftId: UUID? = nil, requestId: UUID? = nil, details: String? = nil) {
        #if DEBUG
        print("[DEBUG] \(message) | Drift: \(driftId?.uuidString ?? "nil") | Req: \(requestId?.uuidString ?? "nil") | Details: \(details ?? "none")")
        #endif
    }
}
